...
# Obtener la ruta del archivo CSV desde los argumentos de la línea de comandos
archivo_csv = sys.argv[1]
ruta1 = os.path.dirname(os.path.abspath(archivo_json))

# Leer el archivo CSV
with open(archivo_csv, 'r', encoding='utf-8') as file:
    reader = csv.reader(file)
    headers = next(reader)  # Leer los encabezados
    rows = [row for row in reader]  # Leer todas las filas

# Crear directorio "Results" si no existe
if not os.path.exists("Results"):
    os.makedirs("Results")

# Verificar si la carpeta "Objects Data" existe
...
# Ruta al archivo que se moverá y modificará
archivo_dat1 = os.path.join(ruta1, 'input2.dat')

# Leer el archivo .dat y almacenar su contenido
if os.path.exists(archivo_dat1):
    with open(archivo_dat1, 'r', encoding='utf-8') as archivo1:
        lineas = archivo1.readlines()
else:
    print(f"Archivo {archivo_dat1} no encontrado.")
    sys.exit()
...
# Procesar cada fila del archivo CSV para crear y modificar copias de input1.dat
for row in rows:
    file_name_with_ext = row[0]  # Nombre del archivo con la extensión de la primera columna
    ...
    SpectrumSize = int(row[4]) - 1
    specsize=str(SpectrumSize)
    #print(SpectrumSize)
    # Modificar los datos del archivo input1.dat
    redshift_value=row[1]
    #Code Part
    datos1[1][0] = iterr #1
    datos1[2][0] = ile #2
    ...
    # Modificar y escribir archivo input2.dat en la carpeta correspondiente
    modified_dat_path = os.path.join(folder_path, 'input2.dat')
    try:
        with open(modified_dat_path, 'w', encoding='utf-8') as archivo_modificado:
            for fila in datos1:
                archivo_modificado.write(' '.join(fila) + '\n')
        #print(f"Archivo modificado guardado en: {modified_dat_path}")
    except IOError as e:
